...
The macOS jobs do require Apple Developer ID signing and notarization secrets. Before the Tauri build, the job runs [`.github/scripts/prepare-macos-signing-env.sh`](../.github/scripts/prepare-macos-signing-env.sh), which validates the required secrets and writes the App Store Connect `.p8` private key into the runner temp directory. The script exposes only `APPLE_API_KEY_PATH` through `GITHUB_ENV`; it must not print certificate, password, or private-key values.
...
Do not add `APPLE_API_KEY_PATH` as a GitHub secret for these workflows. The path is runner-local and generated by `prepare-macos-signing-env.sh` after writing `APPLE_API_PRIVATE_KEY` to a temporary `.p8` file.
