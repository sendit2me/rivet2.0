...
- `Expression`, `JS Filter`, and `JS Map` are the current exceptions to the generic error short-circuit in `NodeOutput`: their custom output renderers still run on failed executions so the red error view can include source-debug sections such as `Parsed expression` when interpolation-created input ports exist
