...
- `Code` node execution diagnostics are split deliberately: [`packages/core/src/model/nodes/CodeNode.ts`](../packages/core/src/model/nodes/CodeNode.ts) enriches user-code runtime and syntax errors with code-node line/column information, while [`packages/app/src/components/nodes/CodeNode.tsx`](../packages/app/src/components/nodes/CodeNode.tsx) renders Code-node failures as a structured red output with the error message plus an `Error location` section. Successful runs do not perform a syntax parse; syntax-location parsing is only attempted after an `AsyncFunction` construction failure.
