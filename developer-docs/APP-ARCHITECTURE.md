...
- the in-app Menu dropdown delegates to the shared menu command surface, so `Save project` is the same command used by app hotkeys and non-Windows native menus. Windows desktop uses this in-strip Menu plus the Windows hotkey shim instead of a native App/File/Edit menubar. Non-Windows Tauri builds keep the native menu path and do not show the in-strip Menu. The active menu-command handler is mirrored on `window` so in-app menu actions, native menu events, and the Windows hotkey shim keep dispatching to the current handler across Vite Fast Refresh updates. The Windows shortcut shim listens on capture-phase `keydown`, prevents the browser default for mapped commands such as `Ctrl+S`, and accepts common Windows user-agent/platform variants rather than only `Win64`. `Ctrl/Cmd+Shift+I` is reserved for browser/WebView developer tools, so Import Graph remains a Menu action without a Windows shim shortcut or Tauri accelerator. The Windows frameless top strip keeps an empty drag region in the opened-project tab area and calls `appWindow.startDragging()`, while double-clicking that empty drag region calls `toggleMaximize()` like a normal Windows titlebar. That drag region must still flex-fill in no-project or auxiliary-workspace states where there are no visible project tabs. The control buttons use `minimize()`, `toggleMaximize()`, and `close()` through the platform window wrapper; do not put those methods directly into unrelated UI code.
...
The left rail uses the same translucent panel background token as the node settings panel, `--grey-dark-bluish-seethrough`, with the same blur strength so canvas-side panels feel like one material system. Major panel boundaries should use a flat `--grey-darkish` line: the left rail owns its right border, and the node settings panel owns its left border without a shadow.
...
- the canvas background color and pattern are persisted UI preferences. `canvasBackgroundColorModeState` selects between the default grey canvas, black canvas, and custom canvas color; `canvasBackgroundCustomColorState` stores a sanitized RGBA string used only when the `Custom` mode is selected. `NodeCanvas` resolves those values into `--canvas-background-color`, while [`packages/app/src/components/nodeCanvas/nodeCanvasStyles.ts`](../packages/app/src/components/nodeCanvas/nodeCanvasStyles.ts) keeps `var(--grey-darker)` as the fallback so old settings and invalid storage still preserve the historical canvas color. `canvasBackgroundPatternState` selects between the grid, dots, and crosses variants, while `canvasBackgroundPatternOpacityState` controls pattern transparency independently from theme selection. The shared `--canvas-background-pattern-rgb` token is defined once in [`packages/app/src/colors.css`](../packages/app/src/colors.css) and must not be overridden by individual themes, so switching Theme never changes the canvas pattern color. [`packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx`](../packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx) draws the pattern into a pointer-transparent canvas behind nodes instead of relying on CSS radial-gradient pixel art; this keeps dots and crosses visible across browser rasterizers. The renderer paints a cached single-tile canvas with `createPattern(...)` and translates that repeated tile for pan/zoom, so zoomed-out cross rendering must stay proportional to canvas area rather than to the number of visible grid intersections. The crosses variant draws a thin 13-pixel plus shape at each grid corner, and the dots variant uses a 2px-by-2px dot footprint. Grid, dots, and crosses intentionally use the same color and opacity setting so switching pattern types changes only geometry, not brightness.
