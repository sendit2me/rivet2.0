...
- nodes with frozen output show a compact snowflake indicator in the node header immediately before the gear control. `VisualNode` derives the persistent `frozen` presentation class from `frozenNodeOutputsState` and the current graph id, while `NormalVisualNodeContent` and `ZoomedOutVisualNodeContent` render the tooltip/icon only when header controls are visible. The icon is a state marker only; execution status colors still come from the normal node run data. Frozen output presentation is blue on the canvas: the header uses the frozen-output accent with a subtle varied-size snowflake background pattern, custom node outlines use the same blue when they are not being overridden by selection/hover/overlay state, and successful frozen output previews use a blue accent/tint for the output stripe, pointer, and output shell. The fullscreen output modal keeps the normal modal colors even when opened from a node with frozen output.
...
Freeze node output is attached only for normal live editor runs. Browser execution clones `frozenNodeOutputsState` and installs the core frozen-output resolver before `processor.processGraph(...)`; recording playback and Trivet/test execution do not receive it. Normal live run-from preload also consults frozen boundary outputs before previous execution history, so a frozen upstream dependency can be reused without adding a new output-history page. Recording playback uses only recording/history data for preload planning.
...
- frozen-output snapshots are sent only to internal executor targets (`internal-desktop` and `internal-hosted`) and only for normal graph runs. External Remote Debugger runs, recording playback, and Trivet/test execution must not receive frozen payloads. The app validates and prepares internal-executor frozen payloads before serializing the run message because the app-to-executor command channel is JSON; explicit `undefined` values are represented with debugger transport sentinels and decoded only on the `frozenNodeOutputs` field in [`packages/node/src/debugger.ts`](../packages/node/src/debugger.ts), while genuinely non-JSON-safe frozen values must fail with a user-visible error before websocket send. The app-executor sidecar attaches the same core frozen-output resolver before `processor.run()`, so Browser and internal Node executor modes share replay semantics for JSON-transportable values.
