...
- `Expression` nodes are the current exception to the generic error short-circuit in `NodeOutput`: their custom output renderer still runs on failed executions so the red error view can show the error plus `Parsed expression`
