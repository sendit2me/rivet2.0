...
- switcher rows with visible labels should use [`packages/app/src/components/LabeledToggle.tsx`](../packages/app/src/components/LabeledToggle.tsx) so the switch stays on the left, the label gets a pointer cursor, and hovering either the label or switch applies the same toggle hover treatment without making unrelated empty row space clickable. The node settings panel, settings modal pages, fullscreen-output Markdown toggle, prompt-designer toggle, and similar labeled toggle rows should share that component instead of pairing raw Atlaskit `Toggle` and `Label` ad hoc.
