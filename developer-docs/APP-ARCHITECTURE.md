...
- `LLM Chat` keeps transport-level controls in the final `Technical details` settings group after all model, provider, generation, output, and provider-advanced sections. `Retry on non-200` retries Vercel AI SDK provider failures whose available HTTP status code is not exactly `200`, using the same minimum repeat count of `1` and configurable cooldown semantics as the `Http Call` node. The retry decision happens inside [`packages/core/src/model/chat-v2/chatV2Pipeline.ts`](../packages/core/src/model/chat-v2/chatV2Pipeline.ts) before [`chatV2Errors.ts`](../packages/core/src/model/chat-v2/chatV2Errors.ts) formats the final error, so exhausted retries still show the provider/model/endpoint guidance and preserve `statusCode` for tests or callers that inspect the thrown error. `Output request status` adds `Request Status` and `Request Error` transport outputs. Successful Vercel SDK calls report `200` and a `control-flow-excluded` `Request Error` because provider non-2XX responses arrive as SDK errors, while custom stream executors can pass through an explicit status. When `Output request status` is enabled and a provider API-call or fetch-layer failure remains after retries, the node returns normally with `Request Error` populated, model-response outputs marked `control-flow-excluded`, and `Request Status` populated when the runtime can see an HTTP status; status-less API-call or fetch-layer failures leave `Request Status` as `control-flow-excluded`. SDK setup/model/validation errors that happen outside that provider request-failure surface still fail the node normally so the setting does not hide configuration or implementation bugs. When `Output request status` is disabled, the same provider request failure stays a normal red node error. `Auto-continue after toolcalls run` receives the same retry behavior on each model round because tool continuation calls the shared pipeline for every provider request.
