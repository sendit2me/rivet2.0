...
- `LLM Chat` places provider-specific `OpenAI`, `Anthropic`, and `Google` editor groups immediately after the `Model` group so provider-only request knobs stay close to provider/model selection. It does not render a provider-specific group for `Custom provider`; the custom-provider contract is the OpenAI-compatible base URL plus generic request settings. The custom provider's `Provider base URL` field lives in `Model`, uses the custom-provider-only `customProviderBaseURL` / `useCustomProviderBaseURLInput` data keys and the separate `customProviderBaseURL` input port, accepts either a base URL such as `https://api.cerebras.ai/v1` or a full `/chat/completions` endpoint, and normalizes that endpoint through [`openAICompatibleEndpointToBaseURL(...)`](../packages/core/src/model/chat-v2/providerOptions.ts). The generic `Provider Advanced` `Base URL` override remains backed by `baseURL` / `useBaseURLInput` plus the `baseURL` input port and is available only for built-in providers, so switching from `Custom provider` back to OpenAI/Anthropic/Google does not turn the custom provider endpoint or input connection into a built-in provider override. Inactive URL fields are also ignored by editor cache identity. It does not expose Google provider-specific `Structured Outputs` because the shared `Response format` group is Rivet's text/JSON/JSON-schema contract.
- `LLM Chat` keeps transport-level controls in the final `Technical details` settings group after all model, provider, generation, output, and provider-advanced sections. `Retry on non-200` retries Vercel AI SDK provider failures whose available HTTP status code is not exactly `200`, using the same minimum repeat count of `1` and configurable cooldown semantics as the `Http Call` node. The retry decision happens inside [`packages/core/src/model/chat-v2/chatV2Pipeline.ts`](../packages/core/src/model/chat-v2/chatV2Pipeline.ts) before [`chatV2Errors.ts`](../packages/core/src/model/chat-v2/chatV2Errors.ts) formats the final error, so exhausted retries still show the provider/model/endpoint guidance and preserve `statusCode` for tests or callers that inspect the thrown error. `Output request status` adds scalar final-response outputs named `Response Status` (`requestStatus`) and `Response Error` (`requestError`). Successful Vercel SDK calls report `200` and a `control-flow-excluded` response error because provider non-2XX responses arrive as SDK errors, while custom stream executors can pass through an explicit status. When `Retry on non-200` is on, the node also adds per-attempt outputs named `Request Statuses` (`requestStatuses`, `number[]`) and `Request Errors` (`requestErrors`, `string[]`); those arrays contain each observable attempt status and each normalized failed-attempt error in order, so a call that succeeds after one retry can return `Response Status = 200`, `Request Statuses = [failedStatus, 200]`, and a one-item `Request Errors` list for the failed attempt. When `Output request status` is enabled and a provider API-call or fetch-layer failure remains after retries, the node returns normally with `Response Error` populated from the final failure, `Request Errors` populated with every failed attempt in retry mode, model-response outputs marked `control-flow-excluded`, and `Response Status` populated when the runtime can see an HTTP status; status-less API-call or fetch-layer failures leave `Response Status` as `control-flow-excluded`. If a custom stream executor completes with a final non-200 status instead of throwing, the final response output is preserved and `Response Error` is populated with the normalized status failure so the scalar response outputs still describe the last attempt. SDK setup/model/validation errors that happen outside that provider request-failure surface still fail the node normally so the setting does not hide configuration or implementation bugs. When `Output request status` is disabled, the same provider request failure stays a normal red node error. `Auto-continue after toolcalls run` receives the same retry behavior on each model round because tool continuation calls the shared pipeline for every provider request.
