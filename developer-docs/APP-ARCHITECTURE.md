...
- nodes with frozen output show a compact snowflake indicator in the node header immediately before the gear control. `VisualNode` derives the persistent `frozen` presentation class from `frozenNodeOutputsState` and the current graph id, while `NormalVisualNodeContent` and `ZoomedOutVisualNodeContent` render the tooltip/icon only when header controls are visible. The icon is a state marker only; execution status colors still come from the normal node run data. Frozen output presentation is blue on the canvas, but it must stay scoped to the output area: successful frozen output previews use a blue accent/tint for the output stripe, pointer, and output shell, and the output viewport carries a faint, irregular, varied-size snowflake background pattern. Frozen state must not recolor the node header or custom node outline, and the fullscreen output modal keeps the normal modal colors even when opened from a node with frozen output.
