...
- `Subgraph` uses an app-side custom node body descriptor for its compact canvas graph selector. The selector mirrors the existing settings-panel `Graph` editor, uses the standard monospace node-body font without a bold override, and writes only `SubGraphNode.data.graphId` through `editNode`, so undo/redo, boundary-port recomputation, settings synchronization, and connection recovery stay on the normal node-edit command path. The selector options come from the current project graph list and are sorted like the settings selector; if a node points at a stale graph id, the canvas selector keeps that missing id visible instead of falling back to another graph. While the selector is focused, it installs the same temporary outside-node `pointerdown` blur listener used by Compare so canvas clicks clear native select focus without every Subgraph node carrying a permanent document listener. Only the selector hit target stops double-click propagation. Keep this as a renderer-only affordance; the core `subGraph` node data shape, project YAML, graph execution, and settings-panel graph selector remain the source of truth.
