...
- [`ProjectSelector`](../packages/app/src/components/ProjectSelector.tsx) is the top app bar. It owns the graph-tree collapse button, graph-history previous/next buttons, in-app File menu, opened-project tabs, inline workspace navigation, and Windows-only frameless window controls. When the graph tree is open and Canvas is active, the file menu and opened-project tabs start to the right of the left panel by reserving the live sidebar width after the graph-tree/history control group; auxiliary workspaces do not reserve that left gap. The `File` trigger should use the same square, full-height top-bar visual language as the inline workspace buttons, with the visible background/borders on the shell, a transparent inner button, and a small minimum width so the label does not collapse to text-only width. The parent top bar, not individual project or workspace tab items, owns the bottom keyline.
...
- The in-app File menu content is modeled as typed canonical groups in [`fileMenuConfiguration.ts`](../packages/app/src/utils/fileMenuConfiguration.ts). `RivetAppHost` can pass `ui.fileMenu.visibleItems` to show only specific stable item ids while preserving Rivet's canonical item order and labels. Current stable ids are `new_project`, `open_project`, `save_project`, `save_project_as`, `import_graph`, `export_graph`, `settings`, and `get_help`; the `settings` item is labeled `Rivet settings` so it is distinct from project/node settings surfaces while keeping the stable command id unchanged. `Help` uses the existing `get_help` command id so wrappers can keep or hide it through the same File menu visibility config. Hidden groups collapse automatically, so separators are rendered only between non-empty groups. This host UI config controls visibility only; command behavior remains owned by [`useMenuCommands`](../packages/app/src/hooks/useMenuCommands.ts). It applies to browser/hosted builds and to the Windows desktop in-strip File menu; non-Windows Tauri builds continue to use their native application menu.
...
- the canvas background pattern is a persisted UI preference with theme-defined hue and settings-defined opacity. `canvasBackgroundPatternState` selects between the grid, dots, and crosses variants, while `canvasBackgroundPatternOpacityState` controls pattern transparency independently from theme selection. Theme CSS may set `--canvas-background-pattern-rgb`, but must not bake opacity into that token. [`packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx`](../packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx) draws the pattern into a pointer-transparent canvas behind nodes instead of relying on CSS radial-gradient pixel art; this keeps dots and crosses visible across browser rasterizers. The renderer paints a cached single-tile canvas with `createPattern(...)` and translates that repeated tile for pan/zoom, so zoomed-out cross rendering must stay proportional to canvas area rather than to the number of visible grid intersections. The crosses variant draws a thin 13-pixel plus shape at each grid corner, and the dots variant uses a 2px-by-2px dot footprint. Grid, dots, and crosses intentionally use the same theme color and opacity setting so switching pattern types changes only geometry, not brightness.
...
- it accepts optional `ui` host policy for wrapper-controlled UI visibility.
  ...
  in-app File menu by stable typed item ids without changing command behavior
