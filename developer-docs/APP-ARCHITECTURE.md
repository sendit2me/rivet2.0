...
- the split-run toggle is presented as a `Run per item` label with a separate helper line explaining that the node runs for each item in the array input; keep that copy scoped to the settings UI because persisted/runtime state still uses `isSplitRun`. Split mode exposes `Max runs` as the total item cap, and parallel mode additionally exposes `Max concurrent runs` as the per-node `splitRunConcurrency` override with a minimum of 2 and a default of the engine's parallel split-run concurrency fallback.
