...
- the `Context` page edits the current project's `projectContextState(projectId)` values. Context values remain per-project app storage, not project-file YAML, but their management UI lives in Settings rather than the left-rail Project settings modal. Context IDs are required, trimmed on save, cannot be saved blank, and cannot duplicate another context ID. Existing blank legacy IDs should remain removable and can be renamed to a valid unique ID without leaving the blank entry behind. Context values are plain visible values; do not reintroduce a hidden/secret toggle in this surface. Keep long context key/value wrapping and visible row actions there.
...
- the Settings modal uses an 80%-window-height shell. Pages with their own dense internal content, currently `Plugins`, opt into the `fill-page` main area so the page consumes the available modal height and owns its internal scrolling instead of leaving a fixed-height list with empty modal space below it.
...
The left rail is the graph navigation panel; it does not render a tab bar. Project metadata lives in [`packages/app/src/components/ProjectInfoModal.tsx`](../packages/app/src/components/ProjectInfoModal.tsx), opened by the `Project settings` button at the top of the graph panel content. Keep project metadata editing out of a dedicated tab so the left rail stays focused on graph navigation.
The project settings modal is a flex column whose section rhythm is owned by bottom margins on each setting block, not top margins on individual controls. Its content order is project name, description, main graph, a divider, MCP configuration, project references, revisions, then the bottom-pinned project plugin usage block. Dividers use matching before/after spacing by combining the previous setting's bottom margin with the divider's own bottom margin. Action rows under simple labels, currently MCP Configuration and Project References, use the shared `project-info-action` class so their label-to-button gap stays consistent. Project Context values are edited from the Settings modal's `Context` page, not from this modal. The project settings modal uses the compact UI font-size token for section text and labels so it matches the larger graph-tree row typography while still scaling from the user's base font-size setting; this includes explicit overrides for Atlaskit inline-edit read views and text inputs because those components pin their own internal font size instead of inheriting from the section. The bottom-pinned project plugin usage block also applies the compact token to its heading, helper text, and plugin rows because it is rendered by a separate component. The project settings modal resets Atlaskit inline-edit `form > Field` top margins at the section wrapper level; keep future settings on bottom margins so the modal has one predictable vertical rhythm. The plugin block is the final section and uses auto top margin so it sits against the bottom of the modal when the project metadata content is short, while still scrolling naturally after the other sections when content is tall. When project plugin usage is empty, the block heading says `No plugins used by this project` and does not render a placeholder list row. The project revision empty state should explain the Git requirement explicitly as `No Git history for this project file`, and loaded project-revision copy should refer to project revisions rather than graph-specific revision searches.
...
The left rail hosts `GraphList`, which is no longer a single all-in-one implementation. Graph CRUD and drag/drop logic have been split into hooks. The first control in the graph panel content is the `Project settings` button; it opens `ProjectInfoModal` without changing the selected graph or graph filter state. The graph context menu order is `Rename`, `Duplicate`, `Graph info`, optional `Make main graph`, then danger `Delete`; `Graph info` opens [`packages/app/src/components/GraphInfoModal.tsx`](../packages/app/src/components/GraphInfoModal.tsx). The modal edits the graph that was right-clicked, not merely the currently open graph, and graph revision filtering receives that explicit graph id. Revision rows should stay non-clickable when the selected graph is not present in that historical project revision rather than assuming the active canvas graph is always available.
...
- `NavigationBar` owns the graph-search panel: it appears centered in the top action row at 30vw, focuses the lighter search input whenever search opens or an already-open search receives `Ctrl/Cmd+F` again, uses background-only input focus styling, stays as an input-only search bar until matches exist, keeps the previous visible results while a changed query is being recomputed so the result area does not flash empty between keystrokes, keeps the panel open when the user erases the search field, discloses when results come from separate-word fallback matching, then persists a draggable bottom-edge max height in `graphSearchPanelHeightState` with an invisible resize edge so short result lists can stay shorter, groups graph sections headed as `Graph <name>` with a lightweight `Graph` label and only the graph name emphasized, handles graph-title open actions, rounded result-row blocks with extra vertical spacing and primary-outline hover/focus feedback, a lighter lower context section when node-content snippets are shown, long-line-safe snippet wrapping, soft-dismisses on outside click/Escape/workspace switches without clearing query, matches, selection, or result scroll position, restores the saved result scroll position only when the panel opens/refocuses or the query changes so active wheel scrolling is not opposed by a repeated `scrollTop` write-back, and only clears graph-search state when the close button is clicked
...
- normal nodes cannot be resized narrower than the width needed for their rendered input/output port labels. [`packages/app/src/utils/nodePortLabelWidth.ts`](../packages/app/src/utils/nodePortLabelWidth.ts) owns the deterministic label-width estimate, including the current UI font scale, [`VisualNode`](../packages/app/src/components/VisualNode.tsx) applies the same minimum as a CSS `min-width` for already-saved narrow nodes in the normal rendering path, and [`NormalVisualNodeContent`](../packages/app/src/components/visualNode/NormalVisualNodeContent.tsx) passes that minimum into the resize math so persisted widths catch up during drag.
