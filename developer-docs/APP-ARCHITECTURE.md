...
- node title and description edits in `NodeEditorGlobalControls.tsx` intentionally apply on every keystroke so the canvas and project state stay live; the title uses a local full-width read/edit control to avoid Atlaskit read-view shrink-wrapping, and both fields keep their pre-edit values so cancel restores the previous metadata
