# Desktop App Architecture (`@rivet2/rivet-app`)
...
`@rivet2/rivet-app` is the interactive IDE layer of the monorepo. It is responsible for:
...
- desktop Node-executor correctness depends on the bundled `app-executor` sidecar staying in lockstep with current app/core source, so the Tauri app now rebuilds `@rivet2/rivet-app-executor` before both `tauri dev` and desktop builds instead of assuming a previously built sidecar is still compatible. If execution semantics in core change while a dev app is already running, restart the Tauri app so the active sidecar process is replaced; a browser refresh alone does not reload an already-running sidecar.
...
The desktop app still depends heavily on `@rivet2/rivet-core`, but `GraphProcessor` is less monolithic than before.

Package-boundary rule:

- app code imports core through `@rivet2/rivet-core`; direct `packages/core/src/...` imports are blocked by the shared ESLint config
...
- the app's Vite config intentionally aliases `@rivet2/rivet-core` to [`packages/core/src/index.ts`](../packages/core/src/index.ts) during app dev/build, so dependencies introduced by browser-reachable core source may also need dependency visibility from [`packages/app/package.json`](../packages/app/package.json). This is why Chat v2's OpenAI-compatible provider SDK and its `zod` peer are listed in both core and app: core owns the runtime import, while app visibility keeps Vite/PnP source resolution working in development.
