...
- large execution payloads are now stored preview-first through that same transform layer: oversized `string`, `string[]`, `object`, `any`, and media outputs can be moved into `globalDataRefs` under stable execution-scoped ref ids instead of being kept inline in reactive node state. Ref-backed values may carry app-provided size hints, but [`globalDataRefs`](../packages/app/src/utils/globals/globalDataRefs.ts) must normalize those hints to a finite positive cache size because valid outputs such as empty chat messages can naturally estimate to zero bytes.
...
- `startDebuggerServer` owns Remote Debugger server keepalive in [`packages/node/src/debugger.ts`](../packages/node/src/debugger.ts). It sends protocol-level WebSocket pings every `DEBUGGER_HEARTBEAT_INTERVAL_MS` and terminates sockets that do not answer within `DEBUGGER_HEARTBEAT_TIMEOUT_MS`, keeping hosted `/ws/latest-debugger` connections active through proxy/CDN idle windows without moving liveness into wrapper UI code or changing external-debugger reconnect policy.
