...
Popup and context-menu styling is centralized in [`packages/app/src/components/PopupMenu.tsx`](../packages/app/src/components/PopupMenu.tsx). New popup menus should compose `PopupMenu`, `PopupMenuItem`, `PopupMenuContainer`, or the exported shared style fragments instead of declaring local menu background, radius, border, shadow, row, icon-slot, separator, or hover CSS. Shared menu surfaces use the opaque theme-tinted `--grey-dark-colorish` token and the same `1px solid var(--settings-collapsible-border)` outline as foldable settings sections; keep portal/global Atlaskit `--ds-border` on that same token so modal/dialog outlines and menu outlines do not drift apart. Reserve `--grey-dark-colorish-seethrough` for the left rail and node settings panel, where the canvas should remain faintly visible behind the panel. `PopupMenuContainer` is the transparent Atlaskit popup wrapper; use it when an Atlaskit `Popup` hosts a menu-like surface so there is not a second visible box around the actual menu. The wrapper and menu surface intentionally suppress their own focus outline because Atlaskit may focus the popup container on first open; keyboard-visible focus should stay on actionable menu rows through `PopupMenuItem`. Shared forwarded-ref menu components must keep explicit `displayName` assignments so the app lint contract and React DevTools names remain stable. Separators should live in the gap between rows through the shared separator styles; do not implement separators by changing the following row's padding or corner radius. Menus using [`useContextMenu`](../packages/app/src/hooks/useContextMenu.ts) close on outside `mousedown` rather than waiting for `click`, so context-menu dismissal happens as soon as the next outside click starts. Duplicate actions in both canvas and graph-list menus use the shared plain two-rectangle [`duplicate-icon.svg`](../packages/app/src/assets/icons/duplicate-icon.svg) asset instead of an image/gallery-style icon, so the command reads as duplication rather than media. Its foreground rectangle uses the menu surface fill to cover the rear rectangle where they overlap, and both rectangles stay slightly squared rather than fully rounded.
...
Graph-tree right-clicks must not force focus into the graph list container. [`GraphList.tsx`](../packages/app/src/components/GraphList.tsx) keeps the container focusable so keyboard actions such as `F2` rename work after a normal primary-button click, but context-menu opening should not route through editor/host focus bridge code while a popup is being opened. Context-menu target discovery in [`useContextMenu.ts`](../packages/app/src/hooks/useContextMenu.ts) also keeps a defensive visited/depth guard while walking `parentElement`: normal DOM parent chains cannot cycle, but unusual event targets from portals, SVG/text nodes, host wrappers, or tests should fail closed instead of risking an infinite parent walk.
...
- the Settings modal uses a viewport-capped shell height instead of the old 80%-window-height shell, so tall settings pages can use more of the window without exceeding it. The left navigation rail and right page column are independent scroll containers: long page content should scroll only the right column, while the left rail stays fixed unless the rail itself is taller than the modal. The Settings left navigation rail is an `aside` wrapper around Atlaskit's `SideNavigation`; it scopes `--ds-surface` and its own background to opaque `--grey-dark-colorish` so the nested Atlaskit navigation does not fall back to an untinted gray surface while the modal itself uses the shared overlay token. It also scopes Atlaskit's selected-navigation tokens (`--ds-border-selected`, `--ds-text-selected`, and selected background tokens) to `--primary`-based values so the selected settings page follows the active Rivet theme instead of Atlaskit's default blue. The rail scrolls vertically only; hide horizontal overflow at the sidebar and nested navigation levels and ellipsize long tab labels instead of showing a horizontal scrollbar. Normal settings pages are capped to 850px content width inside the right column for readability. Pages with their own dense internal content, currently `Plugins`, opt into the `fill-page` main area so the page consumes the available modal height, bypasses the normal width cap, and owns its internal scrolling instead of leaving a fixed-height list with empty modal space below it.
...
- the canvas background color and pattern are persisted UI preferences. `canvasBackgroundColorModeState` selects between `Theme` (`--canvas-background-theme-color`, a dedicated theme-tinted dark canvas color), `Grey-blue` (`#282C34`), and custom canvas color; legacy stored `grey` values resolve to `Theme`, while removed/invalid values such as the old `black` mode also fall back to `Theme`. `canvasBackgroundCustomColorState` stores a sanitized RGBA string used only when the `Custom` mode is selected. `NodeCanvas` resolves those values into `--canvas-background-color`, while [`packages/app/src/components/nodeCanvas/nodeCanvasStyles.ts`](../packages/app/src/components/nodeCanvas/nodeCanvasStyles.ts) keeps `var(--grey-darker)` as the fallback so old settings and invalid storage still preserve the themed canvas color. `canvasBackgroundPatternState` selects between the grid, dots, and crosses variants, while `canvasBackgroundPatternOpacityState` controls pattern transparency independently from theme selection. The shared `--canvas-background-pattern-rgb` token is defined once in [`packages/app/src/colors.css`](../packages/app/src/colors.css) and must not be overridden by individual themes, so switching Theme never changes the canvas pattern color. [`packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx`](../packages/app/src/components/nodeCanvas/CanvasBackgroundPattern.tsx) draws the pattern into a pointer-transparent canvas behind nodes instead of relying on CSS radial-gradient pixel art; this keeps dots and crosses visible across browser rasterizers. The renderer paints a cached single-tile canvas with `createPattern(...)` and translates that repeated tile in the same zoomed screen coordinate space as `canvasToClientPosition(...)`, so background intersections stay locked to node coordinates while panning and zooming instead of drifting as a parallax layer. Zoomed-out cross rendering must stay proportional to canvas area rather than to the number of visible grid intersections. The crosses variant draws a thin 13-pixel plus shape at each grid corner, and the dots variant uses a 2px-by-2px dot footprint. Grid, dots, and crosses intentionally use the same color and opacity setting so switching pattern types changes only geometry, not brightness.
- canvas panning and node dragging share the same closed-hand cursor affordance. `NodeCanvas` adds `dragging-canvas` while the background pan gesture is active and `dragging-node` while `@dnd-kit` owns a node drag; [`nodeCanvasStyles`](../packages/app/src/components/nodeCanvas/nodeCanvasStyles.ts) applies the global `grabbing` cursor to both states so descendants cannot fall back to the default arrow mid-drag.
...
- searchable empty-canvas context-menu results are grouped in [`packages/app/src/components/contextMenuSearchGrouping.ts`](../packages/app/src/components/contextMenuSearchGrouping.ts): the blank-canvas search input uses the node-specific placeholder `Type in node name...`, node/add results stay first, matching `Graph Input` and `Graph Output` node results are promoted to the top of that node/add section with a divider before the remaining node results, graph jumps stay under a dedicated `Go to graphs` section, and graph hits render by graph name only
...
- the conditional input port is rendered in a separate header-level `.node-title-ports` layer, but its left edge should still use the same 12px card-padding geometry as the normal `.node-ports` input rail. The synthetic conditional port intentionally hides the visible `if` label because the standalone port marker is enough on conditional nodes, and conditional headers should not reserve extra left padding for that hidden label. Keep the core `IF_PORT` definition intact so hover/tooling/runtime behavior can still identify the port. Keep that special header-port layer aligned with ordinary input ports rather than tuning it independently.
...
- prompt-interpolation theme expansion should go through `resolveMonacoTheme(...)` in [`packages/app/src/components/codeEditorTheme.ts`](../packages/app/src/components/codeEditorTheme.ts); both the node-editor code path and [`packages/app/src/components/ColorizedPreformattedText.tsx`](../packages/app/src/components/ColorizedPreformattedText.tsx) share that helper instead of duplicating prompt-theme resolution. Register a prompt-interpolation Monaco theme id for every app theme value, including `custom`, so choosing a custom app theme never asks Monaco to apply an undefined theme id.
...
- the non-lazy `LazyCodeEditor` wrapper in [`packages/app/src/components/LazyComponents.tsx`](../packages/app/src/components/LazyComponents.tsx) owns applying the persisted font size and wheel scaling to all Monaco consumers, while native multiline textareas in places like prompt designer, AI assist, and project-creation flows must opt in explicitly through the same hook
...
- it accepts optional `ui` host policy for wrapper-controlled UI visibility.
  ...
  in-app Menu dropdown by stable typed item ids without changing command behavior
