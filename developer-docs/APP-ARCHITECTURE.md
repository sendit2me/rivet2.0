...
The left rail is the graph navigation panel; it does not render a tab bar or its own collapse/expand tab. The top-bar graph-tree toggle in [`ProjectSelector`](../packages/app/src/components/ProjectSelector.tsx) owns changing `sidebarOpenState`, while `LeftSidebar` owns the animated panel shell, resizing, and graph-list content. Project metadata lives in [`packages/app/src/components/ProjectInfoModal.tsx`](../packages/app/src/components/ProjectInfoModal.tsx), opened by the `Project settings` button at the top of the graph panel content. Keep project metadata editing out of a dedicated tab so the left rail stays focused on graph navigation.
