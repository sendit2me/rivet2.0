...
- split-run visual state is rendered by [`packages/app/src/components/VisualNode.tsx`](../packages/app/src/components/VisualNode.tsx) as a centered above-node summary pill (`parallel/sequential, max n`), not inside `NormalVisualNodeContent` / `ZoomedOutVisualNodeContent` headers; the header should stay reserved for title, running state, history-change affordance, and settings, and the right side keeps a stable two-icon action lane so long titles do not rewrap when the hover-revealed gear appears, while lone running indicators keep padded edge spacing
