...
- in the browser build, the `File` menu sits in the same top bar as the opened-project tabs, not part of the centered overlay-tab switcher; its dropdown owns local open state, outside-click dismissal, menu separators, and the browser-visible order `New project`, `Open project`, `Save project`, `Save project as...`, `Import graph`, `Export graph`, `Rivet settings`, `Help`. When the graph panel is open, `ProjectSelector` inserts a sidebar-width spacer after the graph-tree/history controls so the `File` menu and project tabs begin to the right of the left rail instead of painting over the graph-panel column.
...
- [`ProjectSelector`](../packages/app/src/components/ProjectSelector.tsx) is the top app bar. It owns the graph-tree collapse button, graph-history previous/next buttons, browser File menu, opened-project tabs, and inline workspace navigation. When the graph tree is open, the file menu and opened-project tabs start to the right of the left panel by reserving the live sidebar width after the graph-tree/history control group. The browser `File` trigger should use the same square, full-height top-bar visual language as the inline workspace buttons, with the visible background/borders on the shell, a transparent inner button, and a small minimum width so the label does not collapse to text-only width.
