...
- [`ProjectSelector`](../packages/app/src/components/ProjectSelector.tsx) is the top app bar. It owns the graph-tree collapse button, graph-history previous/next buttons, browser File menu, opened-project tabs, and inline workspace navigation. When the graph tree is open and Canvas is active, the file menu and opened-project tabs start to the right of the left panel by reserving the live sidebar width after the graph-tree/history control group; auxiliary workspaces do not reserve that left gap. The browser `File` trigger should use the same square, full-height top-bar visual language as the inline workspace buttons, with the visible background/borders on the shell, a transparent inner button, and a small minimum width so the label does not collapse to text-only width. The parent top bar, not individual project or workspace tab items, owns the bottom keyline.
