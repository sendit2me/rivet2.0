...
- subprocessor event forwarding in [`SubprocessorBridge.ts`](../packages/core/src/model/SubprocessorBridge.ts) intentionally keeps passive process-event forwarding subscribed for the lifetime of the subprocessor object instead of unsubscribing at that subgraph's own `graphFinish`. Race-input graphs can finish successfully after a winning branch while aborting losing branches; terminal events for already-started losing nodes, including ordinary nodes such as Expression and nested Subgraph nodes, may arrive just after the graph-level finish. Forwarding those late terminals keeps Remote Debugger, recordings, and parent graph observers from leaving nodes visually stuck in `running`. Control wiring such as parent/child pause, resume, and abort still cleans up at the subprocessor's own graph terminal event because those listeners are not needed after the graph run ends.
- external Remote Debugger diagnostics live in [`remoteDebuggerDiagnostics.ts`](../packages/app/src/hooks/remoteDebuggerDiagnostics.ts). While an external debugger session is active, [`useRemoteExecutor`](../packages/app/src/hooks/useRemoteExecutor.ts) records a bounded, value-redacted routing trace for incoming process events: message kind, request id, project id, execution ids, node/process ids, dispatch decision, and before/after unscoped routing state. It also keeps a capped per-process lifecycle ledger so large workflows can still show whether a specific `processId` ever received `nodeFinish`, `nodeError`, or `nodeExcluded` after the rolling trace has evicted older rows. The trace is not printed for successful runs. If [`reconcileRunningProcessesAfterSuccessfulDone`](../packages/app/src/hooks/graphExecutionEventHelpers.ts) has to synthesize the missing-terminal-event warning, the app prints a top-level browser-console warning with a copy-friendly multiline report: diagnosis hints, exact and related process lifecycles, exact and related process traces, root-run trace tail, and recent trace tail. It also attaches the same structured data to that warning and dumps a collapsed details group with console tables and raw trace entries. This lets hosted-wrapper debugger issues distinguish "server never sent the terminal event", "the app routed it away", "the terminal arrived under mismatched nested graph metadata", and "the app dispatched it but state/display stayed running" without changing runtime graph execution or websocket payloads.
