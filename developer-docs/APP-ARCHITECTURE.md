...
The left rail is the graph navigation panel; it does not render a tab bar or its own collapse/expand tab. The top-bar graph-tree toggle in [`ProjectSelector`](../packages/app/src/components/ProjectSelector.tsx) owns changing `sidebarOpenState`, while `LeftSidebar` owns the animated panel shell, resizing, and graph-list content. The open/close shell transition intentionally uses `SIDEBAR_TRANSITION_EASING`, a snappy cubic-bezier with a slight overshoot, and disables that transition while the user is resizing the rail. Project metadata lives in [`packages/app/src/components/ProjectInfoModal.tsx`](../packages/app/src/components/ProjectInfoModal.tsx), opened by the `Project settings` button at the top of the graph panel content. Keep project metadata editing out of a dedicated tab so the left rail stays focused on graph navigation.
...
The left rail hosts `GraphList`, which is no longer a single all-in-one implementation. Graph CRUD and drag/drop logic have been split into hooks. The graph panel content starts with a compact `Project:` title row, then plain text-list action rows for `Project settings` and the borderless `Filter graphs` input; these live inside `project-tree-panel-header`, which reuses the node-settings header background token `var(--black-seethrough)`. Keep those controls visually aligned with graph rows instead of bringing back boxed button/input chrome. The `Project settings` icon intentionally keeps a small row-scoped optical vertical adjustment so it aligns with the label while the filter icon stays centered on the input. The filter keeps its placeholder only while unfocused so clicking the filter row leaves a clean empty field, while its `aria-label` remains the persistent accessible name. `Project settings` opens `ProjectInfoModal` without changing the selected graph or graph filter state. Nested graph indentation is applied inside the selectable row with `--graph-item-indent` so selected graph backgrounds span the row width while text, folder icons, and graph names remain indented. The graph context menu order is `Rename`, `Duplicate`, `Graph info`, optional `Make main graph`, then danger `Delete`; `Graph info` opens [`packages/app/src/components/GraphInfoModal.tsx`](../packages/app/src/components/GraphInfoModal.tsx). The modal edits the graph that was right-clicked, not merely the currently open graph, and graph revision filtering receives that explicit graph id. Revision rows should stay non-clickable when the selected graph is not present in that historical project revision rather than assuming the active canvas graph is always available.
...
- node body height caching is only a pending-render placeholder; when a node body resolves to `undefined`, [`useNodeBodyHeight`](../packages/app/src/hooks/useNodeBodyHeight.tsx) must clear the cached height and the empty `.node-body` wrapper must collapse its margin so settings-driven body removal does not leave stale vertical space
...
- in-canvas output content fades in only when a new output body is mounted for a process. When one renderable body replaces another, the old content is removed immediately with no exit transition; brief rerun clear/no-output gaps are handled by the short replacement grace period described in the execution UI section above. The output card shell/border is not animated. Keep this behavior centralized in `NodeOutput.tsx` instead of adding per-node renderer fades, so custom renderers and generic `RenderDataOutputs` share the same no-flash rerun behavior.
