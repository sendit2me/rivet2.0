...
- built-in callback-list nodes such as `jsFilter` and `jsMap` intentionally stay on that generic `type: 'code'` editor path; their "body of `(item, index, array) => { ... }`" UX is a core-node contract created through seeded callback-body text, helper copy, and generated execution wrappers rather than an app-side custom editor. Shared scaffolding for their input definitions, editor definition, body preview, CodeRunner options, and process-time output validation lives in [`packages/core/src/model/nodes/jsListCallbackHelpers.ts`](../packages/core/src/model/nodes/jsListCallbackHelpers.ts), while the filter/map wrapper strings stay explicit so their runtime differences remain easy to inspect. They also support Expression-style `{{var}}` raw-JS-source interpolation in the stored callback body: dynamic interpolation ports are `string` ports, values are inserted as source snippets with no auto-quoting, missing values become `undefined`, and callback-local names (`item`, `index`, and `array`) stay reserved through the exported `JS_LIST_CALLBACK_LOCAL_NAMES` set so runtime interpolation, input-port discovery, and app-side parsed-source display share the same boundary. Callback code should use those existing callback parameters directly rather than `{{item}}`, `{{index}}`, or `{{array}}`. The app gives these nodes a presentation-only output renderer in [`packages/app/src/components/nodes/JSListNode.tsx`](../packages/app/src/components/nodes/JSListNode.tsx): normal output values remain unchanged, and the renderer shows a `Parsed expression` source preview only when the callback body actually defines interpolation-created input ports.
- the built-in `Expression` node stays on that generic `type: 'code'` editor path; its `{{var}}` ports, raw-JS-source interpolation, fixed `output` contract, and disabled-by-default runtime capabilities are all core-node behavior rather than an app-side custom editor. `Expression` deliberately treats interpolation inputs as source snippets, so string literals must include their own quotes and missing inputs resolve to `undefined`. The shared interpolation helper trims the fully substituted expression before execution, and the app reuses that same trimmed source for the debug preview so what the user sees as `Parsed expression` matches what the node actually evaluates. The app does give it a presentation-only custom output renderer in [`packages/app/src/components/nodes/ExpressionNode.tsx`](../packages/app/src/components/nodes/ExpressionNode.tsx): successful runs show `Resulting value`, failed runs keep the red error state and show the error, and both states show the fully substituted `Parsed expression` only when the expression has interpolation-created input ports. That debug view prefers the expression source snapshot captured with the run, so editing the node after execution does not rewrite the historical parsed-expression preview. None of that changes the real graph output contract, which remains one fixed `output` value
...
- prompt-style `{{...}}` input discovery and runtime substitution are owned by [`packages/core/src/utils/interpolation.ts`](../packages/core/src/utils/interpolation.ts); malformed openers like `{{bar` must stay literal instead of swallowing later valid tokens, escaped `{{{...}}}` tokens must round-trip literally, and custom interpolation nodes like [`packages/core/src/model/nodes/ObjectNode.ts`](../packages/core/src/model/nodes/ObjectNode.ts) and [`packages/core/src/model/nodes/ExtractObjectPathNode.ts`](../packages/core/src/model/nodes/ExtractObjectPathNode.ts) should reuse the shared token-boundary helpers rather than reintroducing regex parsing drift
...
- shared structured-output presentation for labeled sections, error text, and colorized parsed-source blocks lives in [`packages/app/src/components/nodes/StructuredNodeOutput.tsx`](../packages/app/src/components/nodes/StructuredNodeOutput.tsx); source-display policy helpers such as [`packages/app/src/components/nodes/parsedSourceDisplayUtils.ts`](../packages/app/src/components/nodes/parsedSourceDisplayUtils.ts) only decide whether a debug source section should be shown and must not perform runtime interpolation
