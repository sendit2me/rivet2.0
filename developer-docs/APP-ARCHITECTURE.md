...
- the `Context` page edits the current project's `projectContextState(projectId)` values. Context values remain per-project app storage, not project-file YAML, but their management UI lives in Settings rather than the left Project tab. Context IDs are required, trimmed on save, cannot be saved blank, and cannot duplicate another context ID. Existing blank legacy IDs should remain removable and can be renamed to a valid unique ID without leaving the blank entry behind. Context values are plain visible values; do not reintroduce a hidden/secret toggle in this surface. Keep long context key/value wrapping and visible row actions there.
...
The visual order is intentionally `Project` / `Graphs`, but `Graphs` remains the default selected tab when a project opens so the graph tree is immediately available.
The `Project` tab is a flex column whose section rhythm is owned by bottom margins on each setting block, not top margins on individual controls. Its content order is project name, description, main graph, a divider, MCP configuration, project references, revisions, then the bottom-pinned project plugin usage block. Dividers use matching before/after spacing by combining the previous setting's bottom margin with the divider's own bottom margin. Action rows under simple labels, currently MCP Configuration and Project References, use the shared `project-info-action` class so their label-to-button gap stays consistent. Project Context values are edited from the Settings modal's `Context` page, not from this tab. The project info tab uses the compact UI font-size token for section text and labels so it matches the larger graph-tree row typography while still scaling from the user's base font-size setting; this includes explicit overrides for Atlaskit inline-edit read views and text inputs because those components pin their own internal font size instead of inheriting from the section. The bottom-pinned project plugin usage block also applies the compact token to its heading, helper text, and plugin rows because it is rendered by a separate component. The project info tab resets Atlaskit inline-edit `form > Field` top margins at the section wrapper level; keep future sidebar settings on bottom margins so the rail has one predictable vertical rhythm. The plugin block is the final section and uses auto top margin so it sits against the bottom of the panel when the project metadata content is short, while still scrolling naturally after the other sections when content is tall. When project plugin usage is empty, the block heading says `No plugins used by this project` and does not render a placeholder list row. The project revision empty state should explain the Git requirement explicitly as `No Git history for this project file`.
...
The `Graphs` tab hosts `GraphList`, which is no longer a single all-in-one implementation. Graph CRUD and drag/drop logic have been split into hooks. The graph context menu order is `Rename`, `Duplicate`, `Graph info`, optional `Make main graph`, then danger `Delete`; `Graph info` opens [`packages/app/src/components/GraphInfoModal.tsx`](../packages/app/src/components/GraphInfoModal.tsx). The modal edits the graph that was right-clicked, not merely the currently open graph, and graph revision filtering receives that explicit graph id. Revision rows should stay non-clickable when the selected graph is not present in that historical project revision rather than assuming the active canvas graph is always available.
