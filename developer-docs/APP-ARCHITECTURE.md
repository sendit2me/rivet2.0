...
- derive CSS variables from node colors; [`nodeColor.ts`](../packages/app/src/utils/nodeColor.ts) is the shared app-side contract for translating saved `{ bg, border }` visual data into header fill, frame behavior, and header foreground contrast. Literal node palette colors and literal saved colors use [`colorContrast.ts`](../packages/app/src/utils/colorContrast.ts) to choose black or white text/icons by WCAG contrast ratio. The uncolored/default node skin renders through the dedicated `--node-color-0` / `--node-color-0-foreground` tokens rather than the generic `--grey-darkish` surface token, so light themes can tune neutral node contrast without changing menu, panel, or modal surfaces. Canvas node bodies use the separate `--node-body-bg` token, and graph-search mini-node content plus the node-color picker preview swatch use that same token so Bright can darken node bodies without also darkening generic forms, modals, or settings panels. Resting node cards use `--node-resting-shadow` instead of a hard-coded shadow so Bright can soften node depth without changing drag overlays, glow states, or menu/action shadows. Saved legacy `var(--grey-darkish)` default node colors are normalized to `--node-color-0` at render time, while the color picker still writes the project-compatible `var(--grey-darkish)` value for the explicit default border+header option.
