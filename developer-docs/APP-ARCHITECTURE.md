...
- frozen nodes show a compact current-color snowflake indicator in the node header immediately before the gear control. `VisualNode` derives the persistent `frozen` presentation class from `frozenNodeOutputsState` and the current graph id, while `NormalVisualNodeContent` and `ZoomedOutVisualNodeContent` render the tooltip/icon only when header controls are visible. The icon is a state marker only; execution status colors still come from the normal node run data. Successful frozen output previews use a blue accent/tint for the output stripe, pointer, and output shell so the reused output reads as frozen without overriding running, error, interrupted, or not-ran status chrome.
