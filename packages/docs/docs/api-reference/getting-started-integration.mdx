...
Welcome to the starting guide for integrating Rivet into your application! Currently, we only support integration via Node.js using the `@rivet2/rivet-node` package.
...
```javascript
import * as Rivet from '@rivet2/rivet-node';
```
...
```typescript
import { runGraphInFile, DataValue } from '@rivet2/rivet-node';
