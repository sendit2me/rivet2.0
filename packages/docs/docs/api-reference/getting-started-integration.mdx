...
- `openAiKey` / `openAiOrganization` / `openAiEndpoint`: Legacy/shared OpenAI settings used by legacy Chat and OpenAI-backed nodes. LLM Chat can also use provider settings or an API Key input port depending on its API key source.
...
request pass its own `inputs`, `context`, and `abortSignal`. The
`headless-fast` profile also caches the graph execution plan, can use a narrow
fast scheduler for eligible acyclic graph shapes, and, when the runner owns the
default CodeRunner, compiled Code-family JavaScript.
