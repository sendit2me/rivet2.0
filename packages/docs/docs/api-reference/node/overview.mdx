...
This is the API overview for the `@rivet2/rivet-node` Node.js library. Each of the exports in the `api.ts` file is documented in its own page, as linked below:
...
Every type and method exported from `@rivet2/rivet-core` is re-exported from `@rivet2/rivet-node` for convenience. See the [core API overview](../core/overview) for more information.
