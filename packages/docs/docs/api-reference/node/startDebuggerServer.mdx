# startDebuggerServer

## Description

The `startDebuggerServer` function starts a Rivet debugger server. By creating a debugger server in your application, you can
connect to it from Rivet to debug graphs running in your application in real-time.

The server sends WebSocket ping frames to connected debugger clients by default. This keeps idle debugger connections alive
through common proxy and CDN idle timeouts without changing the debugger message protocol. Real debugger traffic also counts
as websocket activity while a heartbeat is pending, so a workflow broadcast after an idle period is not terminated by an older
ping timeout.

## Syntax

```typescript
function startDebuggerServer(options?: {
  getClientsForProcessor?: (processor: GraphProcessor, allClients: WebSocket[]) => WebSocket[];
  getProcessorsForClient?: (client: WebSocket, allProcessors: GraphProcessor[]) => GraphProcessor[];
  datasetProvider?: DebuggerDatasetProvider;
  server?: WebSocketServer;
  port?: number;
  host?: string;
  dynamicGraphRun?: DynamicGraphRun;
  allowGraphUpload?: boolean;
  throttlePartialOutputs?: number;
  heartbeatIntervalMs?: number;
  heartbeatTimeoutMs?: number;
}): RivetDebuggerServer;

type DynamicGraphRunOptions = {
  captureNodeTimings?: boolean;
  // plus request id, graph id, graph data, inputs, context, and preload data
};
```

## Parameters

- `options` (Object): The options for starting the debugger server. This should be an object with the following optional properties:

### getClientsForProcessor

To allow multiple debuggers to connect to multiple processors, it is necessary to store a mapping of processors to clients.

This function takes a processor and all clients, and must return the clients for the processor.

If this function is not passed, then the server will assume that there is only one processor and all clients are connected to it.

### getProcessorsForClient

To allow multiple debuggers to connect to multiple processors, it is necessary to store a mapping of processors to clients.

This function takes a client and all processors, and must return the processors for the client.

If this function is not passed, then the server will assume that there is only one processor and all clients are connected to it.

### datasetProvider

Optional dataset bridge used by debugger clients that request dataset data over the debugger socket.

### server

An instance of a WebSocket server. You may use this to override the default server.

### port

The port on which the default server will listen. The default port is 21888.

### host

The host address used when `startDebuggerServer` creates the default WebSocket server. The default host is `localhost`.

### dynamicGraphRun

Implement this function to handle debugger protocol `run` requests from clients that send them. The current Rivet editor hides its normal **Run** buttons while connected as a remote debugger, so editor users normally start the graph from the remote application and watch the live execution in Rivet instead.

The function will be called with the WebSocket client calling the start, the graph ID of the graph to run, and optionally any inputs to run the
graph with. You must construct a Project and run the graph yourself, passing in the debugger server, to connect the execution with the dynamic graph run.

This function does not return anything, as the connection between the graph execution and the debugger server is handled by the `remoteDebugger` parameter
passed to the `runGraph` function.

If a Rivet client sends `captureNodeTimings`, pass that value through to the processor you create. The desktop app uses this for its internal Node executor so node durations are captured only when the `Show node run durations` setting is enabled.

### allowGraphUpload

If this is set to true, then the server will allow uploading graphs to the server. This is useful for debugging graphs that are not part of a project,
or for debugging graphs that are part of a project but have not been saved to disk.

### throttlePartialOutputs

The minimum time in milliseconds between partial-output debugger broadcasts for the same node. The default is 100.

### heartbeatIntervalMs

The interval in milliseconds for server-side WebSocket ping frames sent to connected debugger clients. The default is 30000.
Set this to 0 or a negative value only when your host owns WebSocket liveness itself and you want to disable Rivet's built-in heartbeat.

### heartbeatTimeoutMs

The grace period in milliseconds after a ping before an unanswered debugger socket is terminated. The default is 10000.

## Return

Returns an instance of `RivetDebuggerServer`. This should be passed to the `runGraph` function to allow the graph execution to communicate with the debugger server.

Server-to-client debugger sends are best-effort, including connection-time messages and processor event broadcasts. If a client cannot be written to, the server reports the transport error through the debugger `error` event and terminates only that websocket; graph execution continues.

The public protocol is assembled by `startDebuggerServer`. Internally, heartbeat, best-effort transport sends, and processor attachment cleanup are separate policies so integrations keep the same websocket message schema while the server can clean up stale sockets and processor listeners safely.

## Examples

```typescript
const debuggerServer = startDebuggerServer({
  port: 8080,
  allowGraphUpload: true,
  dynamicGraphRun: async ({ client, graphId, inputs }) => {
    // Implement your dynamic graph run logic here
  },
});
```

## See Also

- [RivetDebuggerServer](./RivetDebuggerServer)
- [DebuggerEvents](./DebuggerEvents)
- WebSocketServer
- GraphProcessor
- GraphId
