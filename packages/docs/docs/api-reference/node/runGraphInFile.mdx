...
- `openAiKey`: Your OpenAI API key. This is used by OpenAI-backed LLM Chat nodes, legacy Chat nodes, Get Embedding nodes, and OpenAI plugin nodes when they are not supplied a key another way.
