...
| OpenAI settings | `OPENAI_API_KEY`, `OPENAI_ORG_ID`, and `OPENAI_ENDPOINT` when unset |
...
context change between runs. With `runtimeProfile: 'headless-fast'`, the runner
also reuses a cached graph execution plan, uses a narrow fast acyclic scheduler
when the graph is eligible, and uses a cached Node CodeRunner for Code-family
nodes unless you pass an explicit `codeRunner`. With experimental
`runtimeProfile: 'native-fast'`, the runner can try the optional native runtime
for a narrow eligible subset and otherwise falls back to the compatible
TypeScript runner while reporting the native decision for diagnostics. Text
processing pipes outside the native parity-tested subset also fall back to
TypeScript. The decision includes `nativeBackend`, so benchmarks can distinguish
the Rust worker from the JS adapter control path.

`createProcessor` accepts a Node-only
`runtimeProfile?: 'compatible' | 'headless-fast'` option. Omitted
`runtimeProfile` uses the safe single-run fast policy: subprocessor
...
fully compatible rollback path, or `runtimeProfile: 'headless-fast'` for the
more aggressive eligible fast path. `runGraph` ignores any untyped
`runtimeProfile` property and uses the compatible path unless a graph shape is
eligible for its internal default-safe one-shot policy.
