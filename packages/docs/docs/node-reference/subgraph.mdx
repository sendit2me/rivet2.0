...
This makes unused outputs faster, but it also means side-effect-only work in an unconnected child branch will not happen. Connect an output to an active node if that branch must run for a global variable, dataset write, event, audio playback, abort, external call, HTTP/LLM call, or Code/Expression side effect.
