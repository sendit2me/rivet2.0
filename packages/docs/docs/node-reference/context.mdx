...
The Context node is not the only way to access context values. You can also access them in the [Text Node](./text.mdx), Prompt Node, Object Node, and some path editors with `{{@context.<id>}}` interpolation. In the [Code Node](./code.mdx), use `context.<id>.value`.
