...
When Rivet is running from a host application running `@rivet2/rivet-core` or `@rivet2/rivet-node`, then you can attach "External Functions" when running graphs or creating a GraphProcessor. The following external function `sum` will sum all of its arguments:

```ts
import { runGraphInFile } from '@rivet2/rivet-node';
