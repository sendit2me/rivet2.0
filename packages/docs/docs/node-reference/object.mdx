...
The JSON Template editor recognizes `{{name}}` as Rivet template syntax. JSON validation still applies to the rest of the template, but syntax markers caused only by interpolation tokens are ignored while you edit.
