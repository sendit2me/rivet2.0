...
A: The node will send the original string to `No Match`. Use this node when you are sure that the string contains only one JSON object or array. For more complex use-cases, you may want to use a [Code Node](./code.mdx) to parse the string in a custom way.
