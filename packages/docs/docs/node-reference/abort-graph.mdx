...
When a graph is successfully aborted, any already-running nodes or nested Subgraph branches that are stopped by that early exit are shown as excluded/not run rather than failed. Error aborts still surface as graph errors.
