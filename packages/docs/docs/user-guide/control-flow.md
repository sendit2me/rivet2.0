...
Use [Did Run](../node-reference/did-run.mdx) when you need to convert "this branch ran at all" into a boolean. It outputs `true` after all connected inputs have run, even if the actual payloads are falsy or empty (`false`, `0`, an empty string, `null`, or an explicit `undefined` value). This is useful when a later node's built-in `If` port should run based on branch completion rather than value truthiness.
...
- [Coalesce](../node-reference/coalesce.mdx) - `control-flow-excluded` values are skipped, and the next input that actually ran is passed through even when its value is falsy, empty, `null`, or `undefined`. Enable `Ignore 'null'` or `Ignore 'undefined'` on the Coalesce node when those payloads should also be skipped.
