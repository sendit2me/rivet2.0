...
test('SettingsModal uses independent viewport-capped column scrolling', () => {
  ...
  assert.match(source, /\.settings-modal-sidebar \{[\s\S]*max-height: 100%;[\s\S]*overflow: auto;/);
  assert.match(source, /\.settings-modal-sidebar > nav \{[\s\S]*background-color: var\(--grey-dark-colorish\);/);
