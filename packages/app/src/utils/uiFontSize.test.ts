...
test('UI font size constants keep the settings slider contract explicit', () => {
  ...
  assert.equal(MIN_UI_FONT_SIZE, 12);
  assert.equal(MAX_UI_FONT_SIZE, 18);
...
test('getUiFontSizeCssVariables scales tokens from the base font size', () => {
  assert.deepEqual(getUiFontSizeCssVariables(18), {
    '--ui-font-scale': String(18 / 14),
    '--ui-font-size-2xs': '13px',
    '--ui-font-size-xs': '14px',
    '--ui-font-size-sm': '15px',
    '--ui-font-size-compact': '17px',
    '--ui-font-size-base': '18px',
    '--ui-font-size-lg': '21px',
    '--ui-font-size-xl': '26px',
    '--ui-font-size-2xl': '31px',
    '--ui-font-size-icon-xl': '41px',
  });
});
